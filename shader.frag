...
vec4 diskColor(vec3 pos, vec3 vel) {
  ...
  if (distToPlane > thickness * 4.0 || r < 2.5 || r > 14.0)
    return vec4(0.0);
  ...
  // The relative emission angle cos_alpha compares photon approach vs fluid
  // velocity
  ...
  float Z_grav =
      sqrt(max(0.001, 1.0 - rs / r_cam)) / sqrt(max(0.001, 1.0 - rs / r));
  ...
  // Blueshifts (g > 1) push towards blue/white, redshifts (g < 1) push towards
  // red/black
...
vec3 render(vec2 in_uv) {
  ...
  // Gravitational aberration (FOV distortion) based on camera depth in
  ...
  for (int i = 0; i < MAX_STEPS; i++) {
    ...
    if (dcol.a > 0.0) {
      ...
      alpha *= (1.0 - dcol.a * dt);
      if (alpha < 0.005)
        break; // Reached full opacity block
    }
...
void main() {
  ...
  vec2 offsets[4] = vec2[](vec2(-0.25, -0.25), vec2(0.25, -0.25),
