...
private:
    ...
    void initWindow() {
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT, "Black Hole Vulkan", nullptr, nullptr);
        
        glfwSetWindowUserPointer(window, this);
        glfwSetCursorPosCallback(window, [](GLFWwindow* w, double x, double y) {
            auto app = reinterpret_cast<BlackHoleRenderer*>(glfwGetWindowUserPointer(w));
    ...
    void onMouseButton(int button, int action, int mods) {
        if (button == GLFW_MOUSE_BUTTON_LEFT) {
            if (action == GLFW_PRESS) mouseDragging = true;
            else if (action == GLFW_RELEASE) mouseDragging = false;
        }
    }
    ...
    void onScroll(double xoffset, double yoffset) {
        ...
        if (cameraRadius < 2.5f) cameraRadius = 2.5f;
    ...
    void createSwapChain() {
        ...
        if (extent.width == UINT32_MAX) extent = {WIDTH, HEIGHT};
    ...
    void drawFrame(ThreadPool& pool) {
        ...
        // This distributes command buffer recordings horizontally if we had more than one
        ...
        // wait for tasks (in a real scenario we wouldn't block, we'd record parallel chunks)
        while(!recorded) { std::this_thread::yield(); }
    ...
    void mainLoop() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (elapsed.count() >= 1.0) {
                std::string title = "Black Hole Vulkan - " + std::to_string(frames) + " FPS";
